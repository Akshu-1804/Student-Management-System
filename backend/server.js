...
pool.query(
    ...
    (err) => {
        if (err) {
            ...
            // Schema Migration: Add columns if they don't exist (for existing tables)
            const migrations = [
                "ALTER TABLE assignments ADD COLUMN assignment_group_id VARCHAR(50) NOT NULL DEFAULT 'LEGACY'",
                "ALTER TABLE assignments ADD COLUMN course_name VARCHAR(100) NOT NULL DEFAULT 'General'",
                "ALTER TABLE assignments ADD COLUMN submission_text TEXT DEFAULT NULL"
            ];
...
// Get assignments (for a specific student or all if admin)
app.get('/api/assignments', (req, res) => {
    const { student_id, view } = req.query;

    if (student_id) { // For student view
        const sql = "SELECT * FROM assignments WHERE student_id = ? ORDER BY assigned_date DESC";
        pool.query(sql, [student_id], (err, results) => {
            if (err) return res.status(500).json({ message: "Error fetching assignments", error: err });
            res.json(results);
        });
    } else if (view === 'admin_grouped') { // For admin grouped view
        const sql = `
            SELECT 
                assignment_group_id, 
                title, 
                ...
                SUM(CASE WHEN status != 'Assigned' THEN 1 ELSE 0 END) as submission_count
            FROM assignments 
            GROUP BY assignment_group_id, title, course_name, due_date, description
            ORDER BY MAX(assigned_date) DESC
        `;
        pool.query(sql, (err, results) => {
            if (err) return res.status(500).json({ message: "Error fetching grouped assignments", error: err });
            res.json(results);
        });
    } else { // Default: flat list for admin if needed, or fallback
        res.status(400).json({ message: "Invalid request parameters" });
    }
});

// Assign a new assignment to a course (Admin)
app.post('/api/assignments', (req, res) => {
    const { course_name, title, description, due_date } = req.body;
    if (!course_name || !title || !due_date) {
        return res.status(400).json({ message: "Course, title, and due date are required." });
    }

    const assignment_group_id = "ASG-" + Date.now() + Math.floor(Math.random() * 1000);

    const findStudentsSql = "SELECT id FROM students WHERE course LIKE ?";
    pool.query(findStudentsSql, [`%${course_name}%`], (err, students) => {
        if (err) return res.status(500).json({ message: "DB error finding students" });
        if (students.length === 0) {
            return res.status(404).json({ message: `No students found enrolled in '${course_name}'.` });
        }

        const insertSql = "INSERT INTO assignments (assignment_group_id, course_name, student_id, title, description, due_date) VALUES ?";
        const values = students.map(student => [
            assignment_group_id, course_name, student.id, title, description, due_date
        ]);
        ...
        pool.query(insertSql, [values], (err, result) => {
            ...
            res.status(201).json({ message: `Assignment created for ${students.length} student(s) in ${course_name}.` });
        });
    });
});

// Update a single assignment submission (Student submit, Admin grade)
app.put('/api/assignments/:id', (req, res) => {
    const { id } = req.params;
    const { status, grade, submission_text } = req.body;
...
// Edit an entire assignment group (Admin)
app.put('/api/assignments/group/:groupId', (req, res) => {
    const { groupId } = req.params;
    const { title, description, due_date } = req.body;
    const sql = "UPDATE assignments SET title = ?, description = ?, due_date = ? WHERE assignment_group_id = ?";
    pool.query(sql, [title, description, due_date, groupId], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ message: "Database error on assignment update" });
        }
        res.json({ message: "Assignment updated successfully" });
    });
});

// Delete an entire assignment group (Admin)
app.delete('/api/assignments/group/:groupId', (req, res) => {
    const { groupId } = req.params;
    const sql = "DELETE FROM assignments WHERE assignment_group_id = ?";
